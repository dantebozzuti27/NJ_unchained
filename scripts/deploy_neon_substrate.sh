#!/usr/bin/env bash
# =============================================================================
# Deploy Phase 4-8c substrate to the production Neon database.
#
...
#   8. Prints verification queries: row counts, function existence, last
...
echo "[2/8] Applying migrations (db/migrations/*.sql)..."
...
echo "[3/8] Applying seeds (db/seeds/*.sql)..."
...
echo "[4/8] Bulk-loading FRED MORTGAGE30US 2010-01-01..2024-12-31..."
"$NJ_FRED" load \
...
echo "[5/8] Bulk-loading ACS5 B19013 (NJ counties, 2010-2024)..."
"$NJ_ACS_INCOME" load \
    --start-year 2010 \
    --end-year   2024 \
    --product    acs5 \
...
echo "[6/8] Bulk-loading NJ DCA county property tax 2016-2024..."
"$NJ_DCA" load \
    --start-year 2016 \
    --end-year   2024 \
    --dsn        "$DSN"
...
echo "[7/8] Bulk-loading NJ DCA municipality property tax 2016-2024..."
"$NJ_DCA" load-muni \
    --start-year 2016 \
    --end-year   2024 \
    --dsn        "$DSN"
...
# ----------------------------------------------------------------------------
# 8. Verification: print row counts + smoke-test the personalization engine
# ----------------------------------------------------------------------------

echo "[8/8] Verifying production state..."
"$PYTHON" - <<'PY'
import os, psycopg
url = os.environ["NEON_DATABASE_URL"]
with psycopg.connect(url, connect_timeout=20) as conn, conn.cursor() as cur:
